...
const createNewDay = (year, day) => {
  fs.mkdir(path.join(__dirname, `input/${year}/${day}`), (err) => {
    if (err) {
      return console.error(err);
    }
  });

  fs.mkdir(path.join(__dirname, `src/${year}/${day}`), (err) => {
    if (err) {
      return console.error(err);
    }
  });

  fs.writeFile(path.join(__dirname, `input/${year}/${day}/input.txt`), "[inset input]", (err) => {
    if (err) {
      return console.error(err);
    }
  });
